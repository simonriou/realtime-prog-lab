...
//  Exercise 1
//
//  Created by Simon Riou on 13/01/2025.
//
...
struct BoundedBuffer* buf_new(int size){
    ...
    unsigned int init_capacity = (unsigned int)size;
    
    sem_unlink("/capacity_sem");
    sem_unlink("/numElements_sem");
    
    buf->capacity = sem_open("/capacity_sem", O_CREAT | O_EXCL, 0644, init_capacity);
    if (buf->capacity == SEM_FAILED) {
        perror("sem_open");
        exit(EXIT_FAILURE);
    }

    buf->numElements = sem_open("/numElements_sem", O_CREAT | O_EXCL, 0644, 0);
    if (buf->numElements == SEM_FAILED) {
        perror("sem_open");
        exit(EXIT_FAILURE);
    }
...
void buf_destroy(struct BoundedBuffer* buf) {
    ...
    sem_close(buf->numElements);
    sem_close(buf->capacity);
    sem_unlink("/numElements_sem");
    sem_unlink("/capacity_sem");
